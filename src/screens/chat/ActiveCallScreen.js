  ...
  useEffect(() => {
    ...
    const startWebRTC = async () => {
      ...
      if (socket) {
        ...
        // If recipient: handle matching offer
        if (!isInitiator && signalData) {
          try {
            await pc.setRemoteDescription(new RTCSessionDescription(signalData));
            const answer = await pc.createAnswer();
            await pc.setLocalDescription(answer);
            socket.emit("accept_call", { targetUserId: profile.id, signalData: answer });
          } catch (e) {
        ...
        if (isInitiator) {
          try {
            ...
            socket.emit("call_user", { targetUserId: profile.id, signalData: offer, callType });
          } catch (e) {
            console.warn('WebRTC initiator handshake offer error:', e.message);
          }

          socket.on("call_accepted", async ({ signalData: answer }) => {
            try {
...
const styles = StyleSheet.create({
  ...
  controlBtn: { width: 56, height: 56, borderRadius: 28, backgroundColor: 'rgba(255,255,255,0.15)', justifyContent: 'center', alignItems: 'center', borderContent: 'solid', borderWidth: 1, borderColor: 'rgba(255,255,255,0.25)' },
