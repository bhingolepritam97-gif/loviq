...
export default function ProfileDetailScreen({ route, navigation }) {
  ...
  if (profile.lookingFor) {
    basics.push({ icon: '💘', label: profile.lookingFor });
  }
    ...
    try {
      ...
      if (result && result.matchedUser) {
        navigation.navigate('Match', {
          matchProfile: result.matchedUser,
          matchId: result.id
        });
      } else {
        navigation.goBack();
    ...
    <>
      ...
      >
        ...
        <View style={[styles.photoContainer, { height: height * 0.75 }]}>
          ...
          <View style={styles.tapNavigationOverlay}>
            <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Previous photo" style={styles.tapAreaHalf} onPress={handlePrevPhoto} activeOpacity={1} />
            <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Next photo" style={styles.tapAreaHalf} onPress={handleNextPhoto} activeOpacity={1} />
          </View>

          {/* Top Controls */}
          <LinearGradient colors={['rgba(0,0,0,0.6)', 'transparent']} style={[styles.topScrim, { paddingTop: insets.top + Spacing.sm }]}>
            <View style={styles.headerControls}>
              <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Go back" style={styles.iconButton} onPress={() => navigation.goBack()}>
                <Ionicons name="chevron-down" size={28} color={Colors.white} />
              </TouchableOpacity>
              <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Safety options menu" style={styles.iconButton} onPress={showSafetyMenu}>
          ...
          <LinearGradient colors={['transparent', 'rgba(0,0,0,0.8)']} style={styles.bottomScrim}>
            ...
            <View style={styles.verifiedFactsStrip}>
              {profile.hasPhone && (
                <View style={styles.verifiedFactBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={Colors.white} />
                  <Text style={styles.verifiedFactText}>Phone verified</Text>
                </View>
              )}
              {profile.isVerified && (
                <View style={styles.verifiedFactBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={Colors.white} />
                  <Text style={styles.verifiedFactText}>Selfie verified</Text>
                </View>
              )}
              {profile.lastActiveAt && (
                <View style={styles.verifiedFactBadge}>
                  <Ionicons name="checkmark-circle" size={14} color={Colors.white} />
                  <Text style={styles.verifiedFactText}>Active recently</Text>
                </View>
              )}
            </View>
        ...
        <View style={styles.contentBody}>
          ...
          {/* Pune Context Section */}
          {(profile.puneNeighborhood || profile.puneSpot) && (
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Pune Local</Text>
              <View style={styles.puneContextContainer}>
                {profile.puneNeighborhood && (
                  <Text style={styles.puneContextText}>
                    <Text style={{fontWeight: 'bold'}}>Neighborhood:</Text> {profile.puneNeighborhood}
                  </Text>
                )}
                {profile.puneSpot && (
                  <Text style={styles.puneContextText}>
                    <Text style={{fontWeight: 'bold'}}>Favorite Spot:</Text> {profile.puneSpot}
                  </Text>
                )}
              </View>
            </View>
            ...
            <View style={styles.section}>
              {prompts.map((prompt, idx) => (
                <TouchableOpacity 
                  ...
                  accessible={true}
                  accessibilityRole="button"
                  accessibilityLabel={`Reply to prompt: ${prompt.question}`}
      ...
      <ReportModal
        ...
        onReportSuccess={() => {
          ...
          Alert.alert('Report submitted', 'We have received your report and will review it shortly. This user has been blocked.');
          navigation.goBack();
        }}
        onBlockSuccess={() => {
          setReportModalVisible(false);
          Alert.alert('User blocked', 'You will no longer see each other.');
          navigation.goBack();
        }}
      />
    </View>
      ...
      >
        ...
        >
          <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Close reply modal" style={styles.modalBackdrop} activeOpacity={1} onPress={() => setReplyPrompt(null)} />
          <View style={[styles.modalContent, { paddingBottom: insets.bottom + Spacing.lg }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Reply to Prompt</Text>
              <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Close prompt reply" onPress={() => setReplyPrompt(null)}>
            ...
            <TouchableOpacity 
              accessible={true}
              accessibilityRole="button"
              accessibilityLabel="Send reply with Super Like"
