...
import { View, Text, StyleSheet, Image, PanResponder, Animated, Dimensions, TouchableOpacity, ActivityIndicator, Modal, KeyboardAvoidingView, Platform, TextInput } from 'react-native';
...
import { subscribeToPotentialMatches, recordSwipe, notifyDeckSwipe, rewindLastSwipe } from '../../services/DiscoverService';
...
export default function DiscoverScreen({ navigation, route }: any) {
  ...
  const [pendingSwipeDirection, setPendingSwipeDirection] = useState<string | null>(null);
  ...
  const handleRewind = () => {
    ...
    if (lastSwipedProfile) {
      setProfiles((prev: any[]) => [lastSwipedProfile, ...prev]);
      setLastSwipedProfile(null);
      if (rewindTimeout.current) clearTimeout(rewindTimeout.current);
    }
  };
  ...
  const handleCallPress = (type: 'audio' | 'video') => {
    // No call from discover screen – navigate to filters or other
  };
  ...
  const handleLikeWithComment = async () => {
    setCommentModalVisible(false);
    if (pendingSwipeDirection) {
      await performSwipe(pendingSwipeDirection, likeComment);
      setLikeComment('');
      setPendingSwipeDirection(null);
    }
  };

  const performSwipe = async (direction: string, comment?: string) => {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {activeFilter && (
        <View style={styles.filterBanner}>
          <View style={styles.filterPill}>
            ...
            <Text style={styles.filterPillText}>{activeFilter.toUpperCase()}</Text>
            <TouchableOpacity style={styles.clearFilterClick} onPress={() => setActiveFilter(null)}>
      ...
      <View style={styles.buttonRow} accessible={false}>
        ...
        >
          <Ionicons name="reload" size={18} color={Colors.gold} />
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <Ionicons name="close" size={28} color={Colors.passRed} />
        </TouchableOpacity>
        <TouchableOpacity 
          ...
          disabled={profiles.length === 0}
        >
          <Ionicons name="star" size={22} color={Colors.superBlue || '#00B4D8'} />
        </TouchableOpacity>
        <TouchableOpacity 
        ...
        >
          <Ionicons name="flash" size={20} color={Colors.boostPurple || Colors.primary} />
        </TouchableOpacity>
      </View>
            ...
            <View style={styles.commentModalContainer}>
              <View style={styles.modalHeaderRow}>
                <Text style={styles.modalTitle}>
                  {pendingSwipeDirection === 'up' ? '⭐ Super Like' : '❤️ Like'} {profiles[0]?.name}
                </Text>
                <TouchableOpacity accessible={true} accessibilityRole="button" accessibilityLabel="Close comment modal" onPress={() => setCommentModalVisible(false)} style={styles.closeModalBtn}>
                  <Ionicons name="close" size={20} color={Colors.text} />
                </TouchableOpacity>
              </View>

              <Text style={styles.modalInstruction}>
                Add a note with your like! (Optional)
              </Text>
              ...
              />
              <Text style={styles.charCountText}>{likeComment.length}/140</Text>

              <View style={styles.modalActionsRow}>
                <TouchableOpacity style={[styles.modalBtn, styles.skipBtn]} onPress={() => handleLikeWithComment()}>
                  <Text style={styles.skipBtnText}>Skip Note</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.modalBtn, styles.sendBtn]} onPress={handleLikeWithComment}>
                  <LinearGradient colors={Gradients.primary.colors as any} start={Gradients.primary.start} end={Gradients.primary.end} style={styles.sendBtnGradient}>
                    <Text style={styles.sendBtnText}>Send Like 💌</Text>
                  </LinearGradient>
                </TouchableOpacity>
              </View>
            </View>
...
const createStyles = (Colors: any) => StyleSheet.create({
  ...
  rewind: { width: 44, height: 44, borderRadius: 22, borderWidth: 1.5, borderColor: Colors.border },
  nope: { width: 62, height: 62, borderRadius: 31, borderWidth: 1.5, borderColor: Colors.border },
  superLike: { width: 48, height: 48, borderRadius: Radius.lg, borderWidth: 1.5, borderColor: Colors.border },
  ...
  boost: { width: 44, height: 44, borderRadius: 22, borderWidth: 1.5, borderColor: Colors.border },
  ...
  filterBanner: { alignItems: 'center', paddingVertical: Spacing.xs, backgroundColor: Colors.background },
  filterPill: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.primary + '20', borderColor: Colors.primary, borderWidth: 1, borderRadius: Radius.full, paddingVertical: 6, paddingHorizontal: 12, gap: 8 },
  filterPillText: { color: Colors.primary, fontSize: 12, fontWeight: '700', letterSpacing: 0.5 },
  clearFilterClick: { padding: 2 },
  clearFilterText: { color: Colors.primary, fontSize: 12, fontWeight: '800' },

  modalWrapper: { flex: 1, justifyContent: 'flex-end' },
  modalBackdrop: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.65)', justifyContent: 'center', alignItems: 'center', padding: Spacing.xl },
  commentModalContainer: { backgroundColor: Colors.surfaceElevated, borderRadius: Radius['2xl'], borderWidth: 1, borderColor: Colors.border, padding: Spacing.xl, width: '100%', maxWidth: 400, ...Shadow.lg },
  modalHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: Spacing.sm },
  modalTitle: { fontSize: 20, fontWeight: '800', color: Colors.text, fontFamily: Typography.fontFamily.serif },
  closeModalBtn: { width: 30, height: 30, borderRadius: 15, backgroundColor: Colors.border, justifyContent: 'center', alignItems: 'center' },
  modalInstruction: { fontSize: 14, color: Colors.textMuted, marginBottom: Spacing.md },
  commentInput: { borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.lg, padding: Spacing.md, height: 90, fontSize: 14, color: Colors.text, textAlignVertical: 'top', backgroundColor: Colors.background, marginBottom: Spacing.xs },
  charCountText: { fontSize: 11, color: Colors.textMuted, textAlign: 'right', marginBottom: Spacing.lg, fontWeight: '600' },
  modalActionsRow: { flexDirection: 'row', gap: Spacing.md },
  modalBtn: { flex: 1, height: 48, borderRadius: Radius.full, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' },
  skipBtn: { backgroundColor: Colors.border },
  skipBtnText: { color: Colors.text, fontSize: 15, fontWeight: '700' },
  sendBtn: { ...Shadow.md },
