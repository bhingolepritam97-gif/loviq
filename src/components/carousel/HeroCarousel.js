  ...
  return (
    <View style={styles.wrapper} {...panResponder.panHandlers}>
      ...
      <View style={styles.pagination}>
        {cards.map((_, index) => (
          <PaginationDot key={index} index={index} activeIndex={activeIndex} itemCount={itemCount} />
        ))}
      </View>
    </View>
  );
...
const PaginationDot = ({ index, activeIndex, itemCount }) => {
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return {
      width: isActive ? 14 : 6,
      backgroundColor: isActive ? '#FF3A5C' : '#E0E0E0'
    };
  });
